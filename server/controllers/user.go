...
func (uc *UserCtrl) Init() error {
	...
	_, err := uc.Coll.Indexes().CreateMany(DefaultCtx(), indexes)
	if err != nil {
		return err
	}
...
func (uc *UserCtrl) RequestPasswordReset(email string) error {
	...
	resetCode := misc.GenerateAlphanumericCode(15)
	mut := bson.D{{"$set", bson.D{{"password_reset_code", resetCode}}}}
	_, err = uc.Coll.UpdateOne(DefaultCtx(), bson.D{{"_id", user.ID}}, mut)
	if err != nil {
		return err
	}
...
func (uc *UserCtrl) ResetPassword(email string, code string) error {
	...
	newPassword := misc.GenerateAlphanumericCode(20)
...
func (uc *UserCtrl) GetAllUsers() ([]models.User, error) {
	users := []models.User{}
	res, err := uc.Coll.Find(DefaultCtx(), bson.D{})
	if err != nil {
		// TODO: wrap
		return nil, err
	}
...
func (uc *UserCtrl) GetUserByID(id string) (*models.User, error) {
	...
	res := uc.Coll.FindOne(DefaultCtx(), bson.D{{"_id", objId}})
	if res.Err() != nil {
		return nil, res.Err()
	}
	user := &models.User{}
...
func (uc *UserCtrl) GetUserByUsername(username string) (*models.User, error) {
	res := uc.Coll.FindOne(DefaultCtx(), bson.D{{"username", username}})
	if res.Err() != nil {
		return nil, res.Err()
	}
	user := &models.User{}
...
func (uc *UserCtrl) GetUserByEmail(email string) (*models.User, error) {
	countByEmail, err := uc.Coll.CountDocuments(DefaultCtx(), bson.D{{"email", email}})
	if err != nil {
		return nil, err
	}
	if countByEmail == 0 {
	...
	res := uc.Coll.FindOne(DefaultCtx(), bson.D{{"email", email}})
	if res.Err() != nil {
		return nil, res.Err()
	}
	user := &models.User{}
...
func (uc *UserCtrl) CreateUser(user *models.User) (*primitive.ObjectID, error) {
	...
	countByEmail, err := uc.Coll.CountDocuments(DefaultCtx(), bson.D{{"email", user.Email}})
	if err != nil {
		return nil, err
	}
	if countByEmail != 0 {
	...
	countByUsername, err := uc.Coll.CountDocuments(DefaultCtx(), bson.D{{"username", user.Username}}, &options.CountOptions{
		Collation: &options.Collation{
	...
	user.ConfirmationCode = misc.GenerateAlphanumericCode(20)
	...
	if _, err := uc.Coll.InsertOne(DefaultCtx(), user); err != nil {
...
func (uc *UserCtrl) UpdateLastAccess(userID string) error {
	...
	_, err = uc.Coll.UpdateOne(DefaultCtx(), bson.D{{"_id", idObj}}, mut)
...
func (uc *UserCtrl) RemoveUser(id string) error {
	...
	_, err = uc.Coll.DeleteOne(DefaultCtx(), bson.D{{"_id", idObj}})
	return errors.Wrapf(err, "(user) couldn't delete user '%s'", id)
}
